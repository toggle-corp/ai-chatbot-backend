...
from user.factories import UserFactory,MemberFactory
...
class TestUserMutations(TestCase):
    ...
    def setUp(self):
        ...
        MemberFactory.create(user=self.admin_user, role=Member.Role.ADMIN)
    ...
    def test_change_password(self):
        ...
        test_user.save()
        # Without authentication 
        response = self._query_change_password(
            "old-password123", "new-password456", assert_errors=True
        )
        assert response["data"] is None
        # With authentication
    ...
    def test_update_me(self):
        ...
        response = self._query_update_me(
            {"firstName": "mandy", "lastName": "handy"}, assert_errors=True
        )
        assert response["data"] is None       
        # With authentication 
