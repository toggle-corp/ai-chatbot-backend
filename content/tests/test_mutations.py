...
class TestContentMutations(TestCase):
    ...
    def setUp(self):
        ...
        self.content1 =  ContentFactory.create(title="Test Document 1",tag=self.tag1,document_status=Content.DocumentStatus.FAILURE) 
        self.content2 = ContentFactory.create(
            document_status=Content.DocumentStatus.TEXT_EXTRACTED,
            is_deleted=False
        )
    ...
    def test_retrigger_content(self, mock_create_embedding_for_content_task):
        ...
        assert resp_data["result"]["title"] == self.content1.title
