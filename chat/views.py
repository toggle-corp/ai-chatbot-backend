...
class UserSession:
    ...
    def create_chat_message(self, data, chat_session):
        user_chat_message = UserChatMessage.objects.create(
            session=chat_session, question=data.get("query"), status=UserChatMessage.Status.STARTED
        )
        return user_chat_message

    def update_chat_message(self, answer, user_message):
        user_message.answer = answer
        user_message.status = UserChatMessage.Status.SUCCESS
        user_message.save(update_fields=["answer", "status"])
        return user_message
